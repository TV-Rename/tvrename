...
namespace TVRename
{
    public class ShowConfiguration : MediaConfiguration
    {
        ...
        public bool NoAirdatesUntilNow(int maxSeasonNumber)
        {
            ...
            if (maxSeasonToUse < 1)
            {
                LOGGER.Error($"{Name} has a problem with series in {Order.PrettyPrint()} order - max={maxSeasonToUse}, keys = {SeasonEpisodes.Keys.ToCsv()}, numberOfEps = {SeasonEpisodes.Count}");
                maxSeasonToUse = 1;
            }
