...
namespace TVRename
{
    ...
    public partial class TmdbCodeFinder : UserControl
    {
        ...
        public TmdbCodeFinder(string? initialHint)
        {
            mInternal = false;
        ...
        public CachedMovieInfo? SelectedShow()
        {
            try
            {
                ...
                return (CachedMovieInfo)lvMatches.SelectedItems[0].Tag;
            }
            catch
            {
                return null;
        ...
        private void Search(bool showErrorMsgBox)
        {
            ...
            txtSearchStatus.Text = "Searching on TMDB.com";
            txtSearchStatus.Update();

            if (!string.IsNullOrEmpty(txtFindThis.Text))
            {
                TMDB.LocalCache.Instance.Search(txtFindThis.Text,showErrorMsgBox); //todo enable search on other sources
                DoFind(true);
            }
        }
