...
namespace TVRename {
    ...
    public:
        ...
        void FillGrid(FNPRegexList ^list)
        {
            ...
            {
                dgv->Rows[i]->Cells[0]->Value = re->RE;
                dgv->Rows[i]->Cells[1]->Value = re->UseFullPath;
                dgv->Rows[i]->Cells[2]->Value = re->Notes;
                i++;
            }
            StartTimer();
        }
...
#pragma region Windows Form Designer generated code
        ...
        void InitializeComponent(void)
        {
            ...
            this->dgv->Columns->AddRange(gcnew cli::array< System::Windows::Forms::DataGridViewColumn^  >(3) {this->TheName, this->Column1, 
                this->Column4});
...
#pragma endregion
    ...
    private: System::Void chkTestAll_CheckedChanged(System::Object^  sender, System::EventArgs^  e) 
             ...
             {
                 if (dgvr->Cells[0]->Value == nullptr) 
                     return nullptr;

                 bool useFolders = (dgvr->Cells[1]->Value == nullptr) ? false : (bool)dgvr->Cells[1]->Value;
                 String ^notes = (dgvr->Cells[2]->Value == nullptr) ? "" : dgvr->Cells[2]->Value->ToString();

                 String ^re = dgvr->Cells[0]->Value->ToString();
                 if (re != "")
                     return gcnew FilenameProcessorRE(re, useFolders, notes);
                 else
                     return nullptr;
             }
             void FillPreview()
             {
                 ...
                 for each (FileInfo ^fi in DirectoryInfo(txtFolder->Text).GetFiles())
                 {
                     ...
                     if (!TheSettings->UsefulExtension(fi->Extension))
                         continue; // move on
