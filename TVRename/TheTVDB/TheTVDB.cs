...
namespace TVRename
{
    ...
    public class TheTVDB : iTVSource
    {
        ...
        public byte[] GetTvdbDownload(string url)
        {
            try
            {
                return TheTVDBAPI.GetTvdbDownload(url, false);
            }
        ...
        private bool UpdateLanguages(bool showErrorMsgBox)
        {
            ...
            try
            {
                JObject jsonLanguagesResponse = TheTVDBAPI.GetLanguages();
        ...
        public bool GetUpdates(bool showErrorMsgBox)
        {
            ...
            while (moreUpdates)
            {
                ...
                try
                {
                    ...
                    jsonUpdateResponse = TheTVDBAPI.GetUpdatesSince(time, lang);
                }
                catch (WebException ex)
                {
                    if (ex.IsUnimportant())
        ...
        private List<JObject> GetEpisodes(int id, string lang)
        {
            ...
            while (morePages)
            {
                try
                {
                    JObject jsonEpisodeResponse = TheTVDBAPI.GetSeriesEpisodes(id, lang, pageNumber);
        ...
        private JObject DownloadSeriesJson(int code, string requestedLanguageCode)
        {
            ...
            try
            {
                jsonResponse = TheTVDBAPI.GetSeries(code, requestedLanguageCode);
            }
            catch (WebException ex)
            {
                    ...
                    resp.StatusCode == HttpStatusCode.NotFound)
                {
                    ...
                    if (TheTVDBAPI.TvdbIsUp() && !CanFindEpisodesFor(code, requestedLanguageCode))
                    {
                        LastErrorMessage = ex.Message;
        ...
        private bool CanFindEpisodesFor(int code, string requestedLanguageCode)
        {
            try
            {
                TheTVDBAPI.GetSeriesEpisodes(code, requestedLanguageCode);
            }
            catch (WebException ex)
            {
                if (ex.Status == WebExceptionStatus.ProtocolError && ex.Response != null &&
        ...
        private void DownloadSeriesActors(int code)
        {
            ...
            try
            {
                ...
                JObject jsonActorsResponse = TheTVDBAPI.GetSeriesActors(code);
            ...
            string requestedLanguageCode)
        {
            ...
            IEnumerable<string> imageTypes = TheTVDBAPI.GetImageTypes(code, requestedLanguageCode);

            List<JObject> bannerResponses = TheTVDBAPI.GetImages(code, requestedLanguageCode, imageTypes);
            ...
            IEnumerable<string> imageDefaultLangTypes = TheTVDBAPI.GetImageTypes(code, DefaultLanguageCode);

            List<JObject> bannerDefaultLangResponses =
                TheTVDBAPI.GetImages(code, DefaultLanguageCode, imageDefaultLangTypes);
        ...
        private bool DownloadEpisodeNow(int seriesId, int episodeId, string requestLangCode, bool dvdOrder = false)
        {
            ...
            try
            {
                jsonEpisodeResponse = TheTVDBAPI.GetEpisode(episodeId, requestLangCode);

                if (IsNotDefaultLanguage(requestLangCode))
                {
                    jsonEpisodeDefaultLangResponse = TheTVDBAPI.GetEpisode(episodeId, DefaultLanguageCode);
                }
            }
            catch (WebException ex)
            {
        ...
        public void Search(string text, bool showErrorMsgBox)
        {
            ...
            try
            {
                jsonSearchResponse = TheTVDBAPI.Search(text, TVSettings.Instance.PreferredLanguageCode);
            }
            catch (WebException ex)
            {
                if (ex.Response is null) //probably a timeout
            ...
            if (InForeignLanguage())
            {
                try
                {
                    jsonSearchDefaultLangResponse = TheTVDBAPI.Search(text, DefaultLanguageCode);
                }
                catch (WebException ex)
                {
                    if (ex.Response is null) //probably a timeout
