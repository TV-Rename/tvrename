...
namespace TVRename
{
    ...
    public interface Action // Something we can do
    {
        ...
        bool Go(Config settings, ref bool pause); // action the action.  do not return until done.  will be run in a dedicated thread.  if pause is set to true, stop working until it goes back to false        
    }
