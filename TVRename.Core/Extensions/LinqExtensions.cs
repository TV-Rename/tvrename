...
namespace TVRename.Core.Extensions
{
    ...
    public static class LinqExtensions
    {
        ...
        {
            return source.ForEachAsync(action, CancellationToken.None, maxDegreeOfParallelism, continueOnCapturedContext);
        }
        ...
        /// <param name="maxDegreeOfParallelism">The maximum degree of parallelism.</param>
        /// <param name="continueOnCapturedContext"><see langword="true" /> to attempt to marshal the continuation back to the original context captured; otherwise, <see langword="false" />.</param>
        public static Task ForEachAsync<TSource>(this IEnumerable<TSource> source, Func<TSource, Task> action, CancellationToken ct, int maxDegreeOfParallelism = 5, bool continueOnCapturedContext = false)
        {
            ...
            {
                await throttler.WaitAsync(ct).ConfigureAwait(continueOnCapturedContext);
            ...
            return Task.WhenAny(Task.WhenAll(tasks), ct.AsTask());
        }
    }
}
